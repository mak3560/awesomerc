...
function magnifier.arrange(p)
    ...
    local geometry = {}
    -- if #cls > 1 then
        geometry.width = area.width * 0.598
        geometry.height = area.height * 0.778
        geometry.x = area.x + (area.width - geometry.width) / 3
        geometry.y = area.y + 10
    -- else
    --     geometry.x = area.x
    --     geometry.y = area.y
    ...
    if #cls > 1 then
        geometry.x = area.x
        geometry.y = area.y + 10
        geometry.height = area.height * 0.8
        geometry.width = area.width / (#cls - 1)
